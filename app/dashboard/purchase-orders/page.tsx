  ...
  return (
    <div className="space-y-4" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <Card className="p-3 flex flex-wrap items-center gap-2">
        ...
        <Select value={supFilter} onChange={(e) => setSupFilter(e.target.value)} className="w-auto"><option value="">{t('po.allSuppliers')}</option>{suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</Select>
        <Select value={vesFilter} onChange={(e) => setVesFilter(e.target.value)} className="w-auto"><option value="">{t('po.allVessels')}</option>{vessels.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</Select>
        <Select value={invFilter} onChange={(e) => setInvFilter(e.target.value as any)} className="w-auto"><option value="all">{t('sup.all')}</option><option value="invoiced">{t('po.invoiced')}</option><option value="none">{t('po.notInvoiced')}</option></Select>
        <Select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="w-auto"><option value="newest">{t('po.sortNewest')}</option><option value="oldest">{t('po.sortOldest')}</option><option value="value">{t('po.sortValue')}</option><option value="number">{t('po.sortNumber')}</option></Select>
