...
const SUGGESTIONS = [
  'كام فاتورة غير مدفوعة وإجمالي المتبقي بكل عملة؟',
  'إيه الفواتير المتأخرة؟',
  'الفواتير اللي حالتها Waiting PO',
];

export default function InvoiceAssistant({ onChanged }: { onChanged: () => void }) {
  ...
  async function send(text?: string) {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/invoices/assistant', { message, history });
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50">
            {msgs.length === 0 && (
              <div className="text-center text-gray-400 text-xs mt-6 space-y-3">
                <p>اسألني تحليل عن الفواتير، أو خليني أحدّث حالة فاتورة أو أضيف تعليق.</p>
                <p className="text-[10px] text-gray-300">بحلّل وبعدّل حالات الموافقة فقط — مش بعدّل مبالغ أو دفعات.</p>
                <div className="space-y-1.5">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="block w-full text-right bg-white border border-gray-200 rounded-lg px-3 py-2 text-[11px] text-gray-600 hover:border-blue-300 hover:text-blue-700 transition"
                    >
                      {s}
                    </button>
                  ))}
          ...
          <div className="p-2 border-t bg-white rounded-b-2xl">
            <div className="flex gap-2">
              <input
                ...
                disabled={loading}
                className="flex-1 border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-50"
              />
              <button
                onClick={() => send()}
                disabled={loading || !input.trim()}
                className="bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white px-4 rounded-xl text-sm font-medium transition"
              >
                إرسال
              </button>
            </div>
